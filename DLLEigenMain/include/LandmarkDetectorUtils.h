...
namespace LandmarkDetector
{
	...
	std::vector<cv::Point2f> CalculateAllParts(const CLNF_EYE& clnf_model);
	std::vector<cv::Point2f> CalculateAllEyeParts(const CLNF_EYE& clnf_model);
	std::vector<cv::Point3f> Calculate3DEyeParts(const CLNF_EYE& clnf_model, float fx, float fy, float cx, float cy);
