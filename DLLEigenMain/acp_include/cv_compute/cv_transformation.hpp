...
public:
    ...
    void run(DataList& in_data_list, DataList& out_data_list)
    {
		...
		std::cout << "CenterCropResize" << std::endl;
    }
};

class PaddingResize : public PipeProcessor {
public:
	int cropHeightLowBnd, cropWidthLowBnd;
	//int srcHeight, srcWidth;
	...
	int dstHeight, dstWidth;
	bool flip, returnCrop;
	PaddingResize(int dst_height, int dst_width, bool flip_img = false, bool return_crop = false, string pp_name = "") :
		PipeProcessor(1, 1, AVP_MAT, pp_name, STREAM_PROC), //srcHeight(src_height), srcWidth(src_width),
		dstHeight(dst_height), dstWidth(dst_width), flip(flip_img), returnCrop(return_crop)
	{
		//ROI = cv::Rect(cropWidthLowBnd, cropHeightLowBnd, cropWidth, cropHeight);
	...
	void run(DataList& in_data_list, DataList& out_data_list)
	{
		for (auto &in_mat : in_data_list[0].matList)
		{
			Mat tmp_mat, padding_mat;
			int srcHeight = in_mat.rows;
			int srcWidth = in_mat.cols;
			float ratio = 1.0 * dstWidth / dstHeight;
			if (srcWidth * dstHeight > srcHeight * dstWidth) //Need to fill the pixels in the height direction
			{
				paddingWidth = srcWidth;
				paddingHeight = paddingWidth / ratio;
			}
			else //Need to fill the pixels in the width direction
			{
				paddingHeight = srcHeight;
				paddingWidth = paddingHeight * ratio;
			}
			padding_mat = Mat::zeros(paddingWidth, paddingHeight, CV_8UC3);
			in_mat.copyTo(padding_mat(cv::Rect(int((paddingWidth - srcWidth) / 2),
				int((paddingHeight - srcHeight) / 2), srcWidth, srcHeight)));
			cv::resize(padding_mat, tmp_mat, { dstWidth, dstHeight }, 0, 0, cv::INTER_LINEAR);
			/*cv::imwrite("image.jpg", in_mat);
			cv::imwrite("padding_mat.jpg", padding_mat);*/
			if (flip)
				cv::flip(tmp_mat, tmp_mat, +1);
			out_data_list[0].loadData(tmp_mat);
			if (returnCrop)
				out_data_list[1].loadData(padding_mat);
		}
	}
};
