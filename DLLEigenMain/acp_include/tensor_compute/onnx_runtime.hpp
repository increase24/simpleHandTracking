...
public:
        ...
        memInfo(Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault)), env(ORT_LOGGING_LEVEL_WARNING, "test")
    {
        ...
        /* modified by jz to adjust onnxruntime in windows */
		//const char* modelPath = model_path.c_str();
		std::wstring widstr = std::wstring(model_path.begin(), model_path.end());
    ...
    void run(DataList& in_data_list, DataList& out_data_list)
    {
		...
		///std::cout << "out_data_list[1]:" << out_data_list[1].tensor() << std::endl;
    }
    /* ------------Potentially Useful------------------------ */
    // Ort::Env* env_ptr;
